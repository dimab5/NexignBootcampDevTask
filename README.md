...
К сожалению, сервисы запускаются только в ручном режиме, когда запуска осуществляется через docker-compose почему-то не работает кафка и не получится протестировать приложение всё приложение, получится только протестировать `CRM`, то есть поотправлять запросы от абонента или от менеджера. 
